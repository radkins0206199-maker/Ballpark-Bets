...
def update_results_tracker(games_list):
    ...
    Stores ML feature columns alongside display columns for auto-recalibration.
    Returns the full combined DataFrame.
    """
...
def auto_fill_results():
    ...
    Fetches yesterday's final scores from MLB API and writes the actual
    winner into any unresolved rows in results_tracker.csv.
    """
    if not os.path.exists(TRACKER_CSV):
        return
    yesterday = (datetime.date.today() - datetime.timedelta(days=1)).isoformat()
    data = _get(f"{MLB_BASE}/schedule",
                {"sportId": 1, "date": yesterday, "hydrate": "linescore,team"})
    if not data:
        return
    ...
    if not results:
        print(f"  [Tracker] No final scores available yet for {yesterday}")
        return

    df      = pd.read_csv(TRACKER_CSV, dtype=str)
    updated = 0
    for idx, row in df.iterrows():
        if str(row.get("Date", "")).strip() != yesterday:
            continue
        if str(row.get("Actual Winner", "")).strip() not in ("", "nan"):
            continue
        key = (str(row.get("Home Team", "")).strip(),
               str(row.get("Away Team", "")).strip())
        if key in results:
            df.at[idx, "Actual Winner"] = results[key]
            updated += 1

    if updated:
        df.to_csv(TRACKER_CSV, index=False)
        print(f"  [Tracker] Auto-filled {updated} result(s) from {yesterday}")
    else:
        print(f"  [Tracker] No unresolved games found for {yesterday}")
...
def retrain_model_if_needed():
    ...
    try:
        ...
        with open(RETRAIN_FLAG, "w") as f:
            f.write(f"{datetime.date.today().isoformat()}:{current_count}")
...
def email_report(report_path):
    ...
    with open(report_path, "rb") as f:
        msg.add_attachment(
            ...
            filename=f"mlb_report_{datetime.date.today().isoformat()}.xlsx"
        )
