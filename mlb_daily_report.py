...
def capture_closing_lines():
    ...
    closing price once stored.
        ...
        CLV = closing_implied - opening_implied   (vig-free, on the picked side)

    Market-to-market. Positive means the line moved toward our pick after we
    made it — the market later agreed with us. Deliberately does NOT reference
    model probability; that would just restate the model's own edge.
    """
    ...
    # Check for pending work BEFORE fetching odds. Each fetch costs an Odds
    # API credit, and the late passes usually have nothing left to capture —
    # on 2026-08-31 the 500-credit monthly allowance ran out, which knocked
    # out live Vegas lines and left that day's games recorded on win%
    # fallback. Querying first turns those wasted calls into free no-ops.
    try:
        import psycopg2
        ...
        """, (today,))
        pending_rows = _cur.fetchone()[0]
        _cur.close(); _c.close()
        if pending_rows == 0:
            print("  [CLV] All of today's games already captured — no odds fetch needed")
            return {"captured": 0, "pending": 0}
    except Exception as e:
        print(f"  [CLV] Pending check failed ({e}) — proceeding with fetch")
...
def retrain_model_if_needed():
    ...
    try:
        ...
        # Core feature set. temp / wind_speed / wind_dir_out / home /
        # pitcher_recent_delta / opp_pitcher_recent_delta were removed on
        # 2026-08-31: they were never written to results rows, so every
        # Cohort B row had them NULL -> fillna(0) -> constant, which the
        # health gate correctly refused to train on.
        #
        # They are now written going forward (see _save_game_features_to_results)
        # and can be restored once ~200 Cohort B rows carry real values. Holding
        # the retrain hostage to them was not worth it: every feature-importance
        # report has been dominated by pitching (opp_pitcher_era ~18%,
        # pitcher_era ~17%, opp_bullpen_era ~14%) with weather at the bottom,
        # and `home` carries almost no signal in this sample (home teams won
        # 50.2% over 1,072 games vs the ~54% league norm).
        #
        # At 268 training rows, fewer features is also simply better: 10 features
        # is ~27 rows each, already thin. 16 would be worse.
        ml_feat_cols = [
        ...
        def _assert_feature_health(df: "pd.DataFrame", cols: list) -> list:
            ...
            RANGES = {
                ...
                "elo_diff": (-800.0, 800.0),   # widened: season-long Elo spread exceeds +/-600 by August
...
def _sf_bool(v):
    """
    wind_dir_out is a BOOLEAN column in Postgres. Writing it via sf() produced
    a float and psycopg2 raised "column is of type boolean but expression is
    of type numeric", which aborted the whole results feature save on
    2026-09-01 and left that day with no rows at all.
    """
    if v is None:
        return None
    if isinstance(v, bool):
        return v
    if isinstance(v, str):
        return v.strip().lower() in ("1", "true", "t", "yes", "out")
    try:
        return bool(float(v))
    except Exception:
        return None
    ...
    try:
        ...
        for g in games:
            ...
            """, (
                ...
                # These six were in ml_feat_cols but never written here, so every
                # Cohort B row had them NULL -> fillna(0) -> constant. Same bug
                # class as the original team-quality failure; caught this time by
                # the feature health gate instead of running silently.
                sf(g.get('temp')),
                sf(g.get('wind_speed')),
                _sf_bool(g.get('wind_dir_out')),
                _sf_bool(g.get('home')),
                sf(g.get('pitcher_recent_delta')),
                sf(g.get('opp_pitcher_recent_delta')),
                date, home, away
            ))
                ...
                cur.execute("""
                    INSERT INTO results
                        (date, home_team, away_team, predicted_winner, confidence,
                         ...
                         team_wpct, opp_wpct, team_rpg, opp_rpg,
                         home_elo, away_elo, elo_diff, vegas_implied,
                         temp, wind_speed, wind_dir_out, home,
                         pitcher_recent_delta, opp_pitcher_recent_delta, model_cohort)
                    ...
                    ON CONFLICT (date, home_team, away_team) DO UPDATE SET
                        ...
                        wind_dir_out     = EXCLUDED.wind_dir_out,
                        home             = EXCLUDED.home,
                        pitcher_recent_delta     = EXCLUDED.pitcher_recent_delta,
                        opp_pitcher_recent_delta = EXCLUDED.opp_pitcher_recent_delta,
                        model_cohort     = 'B'
                """, (
                    date, home, away,
                    g.get('Predicted Winner') or None,
