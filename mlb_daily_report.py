...
def _load_resolved_from_supabase():
    ...
    try:
        ...
        # Rename to match CSV column names
        df = df.rename(columns={
            ...
            'home_sp_era':      'pitcher_era',
            'away_sp_era':      'opp_pitcher_era',
        })
        print(f"  [ML] Loaded {len(df)} resolved rows from Supabase for retraining")
        return df
    except Exception as e:
        print(f"  [ML] Supabase load failed: {e}")
        return None
...
def retrain_model_if_needed():
    ...
    try:
        ...
        # Build training rows — use whatever features are available, fill missing with 0
        live_rows = []
        for _, row in resolved.iterrows():
            try:
                ...
                for f in ml_feat_cols:
                    val = row.get(f, None)
                    try:
                        feat[f] = float(val) if val is not None and str(val).strip() not in ('', 'nan', 'None') else 0.0
                    except (ValueError, TypeError):
                        feat[f] = 0.0
        ...
        if not live_rows:
            print(f"  [ML] Retrain skipped — no valid training rows built")
            return
