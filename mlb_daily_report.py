            ...
            for date_str, rows in dates_to_resolve.items():
                ...
                for row in rows:
                    ...
                    if key in results:
                        ...
                        patch_url = f"{sb_url}/rest/v1/results?date=eq.{date_str}&home_team=eq.{key[0]}&away_team=eq.{key[1]}"
                        patch_body = json.dumps({"actual_winner": actual, "correct": correct}).encode()
                        patch_req = _req.Request(patch_url, data=patch_body, method="PATCH")
                        patch_req.add_header("apikey", sb_key)
                        patch_req.add_header("Authorization", f"Bearer {sb_key}")
                        patch_req.add_header("Content-Type", "application/json")
                        patch_req.add_header("Prefer", "return=minimal")
                        try:
                            with _req.urlopen(patch_req, timeout=8):
...
def _save_predictions_supabase(payload):
    ...
    # 1. Save to predictions table — DELETE then INSERT to avoid 409 conflicts
    try:
        date_str = payload["date"]
        # Step 1a: Delete existing row for today
        del_url = f"{sb_url}/rest/v1/predictions?date=eq.{date_str}"
        del_req = _req.Request(del_url, method="DELETE")
        del_req.add_header("apikey", sb_key)
        del_req.add_header("Authorization", f"Bearer {sb_key}")
        try:
            with _req.urlopen(del_req, timeout=5):
                pass
        except Exception:
            pass  # OK if no row exists yet

        # Step 1b: Insert fresh row
        body = json.dumps({
            "date":          date_str,
            "games":         payload["games"],
        ...
        ins_url = f"{sb_url}/rest/v1/predictions"
        ins_req = _req.Request(ins_url, data=body, method="POST")
        ins_req.add_header("apikey", sb_key)
        ins_req.add_header("Authorization", f"Bearer {sb_key}")
        ins_req.add_header("Content-Type", "application/json")
        ins_req.add_header("Prefer", "return=minimal")
        with _req.urlopen(ins_req, timeout=10) as resp:
            print(f"  [Supabase] ✅ Predictions saved for {date_str} (HTTP {resp.status})")
    except Exception as e:
