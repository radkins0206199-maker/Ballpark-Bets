...
def capture_closing_lines():
    ...
    try:
        ...
        for home, away, game_time, home_prob in rows:
            ...
            # CLV from the model's own side, not always the home side:
            # a pick on the away team has the mirrored edge.
            clv = None
            if home_prob is not None:
                model_home = float(home_prob) / 100.0
                clv = round(model_home - float(closing_prob), 4)
...
def clv_summary(days: int = 30) -> dict:
    ...
    try:
        ...
        cur  = conn.cursor()
        cur.execute("""
            SELECT home_team, predicted_winner, home_win_prob,
                   closing_home_implied, model_cohort
            FROM results
        ...
        for home, pick, hp, close_p, cohort in rows:
            model_home = float(hp) / 100.0
            close_home = float(close_p)
            # Mirror to the picked side
            if pick == home:
                v = model_home - close_home
            else:
                v = (1.0 - model_home) - (1.0 - close_home)
            picked_clv.append(v)
            if cohort == "B":
