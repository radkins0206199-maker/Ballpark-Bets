...
def retrain_model_if_needed():
    ...
    try:
        ...
        # ── Phase 2: Proper holdout evaluation ────────────────────────────────
        # Separate training Brier from holdout Brier
        # Training Brier = how well model fits known data (optimistic)
        # Holdout Brier  = honest estimate of real-world performance
        try:
            import numpy as np
            if len(y_te) >= 10:
                rf_te  = cal_rf.predict_proba(X_te)[:, 1]
                xgb_te = cal_xgb.predict_proba(X_te)[:, 1]
                holdout_brier = float(np.mean((rf_te - np.array(y_te))**2))
                train_rf = cal_rf.predict_proba(X_tr)[:, 1]
                train_brier = float(np.mean((train_rf - np.array(y_tr))**2))
                overfit_gap = holdout_brier - train_brier
                accuracy = sum(cal_rf.predict(X_te) == y_te) / len(y_te)
                print(f"  [ML] Train Brier: {train_brier:.4f} | Holdout Brier: {holdout_brier:.4f} | Gap: {overfit_gap:+.4f}")
                if overfit_gap > 0.03:
                    print(f"  [ML] ⚠️ Overfitting detected (gap {overfit_gap:.4f} > 0.03) — consider more regularization")
            ...
            accuracy = 0.0
            print(f"  [ML] Holdout evaluation failed: {e}")
