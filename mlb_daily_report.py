...
def _load_resolved_from_supabase():
    """Load resolved results from Supabase for model retraining using direct PostgreSQL."""
    try:
        import psycopg2, psycopg2.extras
        db_url = os.environ.get("DATABASE_URL", "")
        if not db_url:
        ...
        for ssl_mode in ['require', 'prefer', 'disable']:
            try:
                conn = psycopg2.connect(db_url, sslmode=ssl_mode, connect_timeout=10)
                print(f"  [ML] Connected to Supabase (sslmode={ssl_mode})", flush=True)
                break
            except Exception as e:
                print(f"  [ML] sslmode={ssl_mode} failed: {e}", flush=True)

        if not conn:
            print("  [ML] All Supabase connections failed", flush=True)
            return None
        ...
        cur.close(); conn.close()
        if not rows:
            print("  [ML] No resolved rows in Supabase", flush=True)
            return None
        ...
        print(f"  [ML] Supabase load failed: {e}", flush=True)
        return None
...
def retrain_model_if_needed():
    ...
    try:
        ...
        # Rows inserted manually (May 9-13) have all-NULL features, skip them
        live_rows = []
        skipped = 0
        for _, row in resolved.iterrows():
            try:
                ...
                # Skip rows with no real feature data (all zeros = manually inserted)
                if non_zero < 3:
                    skipped += 1
                    continue
        ...
        if not live_rows:
            ...
            print(f"  [ML] Note: Need picks generated by Render (not manually inserted) to retrain")
            return
...
def final_win_probability(adv_prob, ml_prob, vegas_prob=None, vegas_is_live=False,
                          platoon_adj=0.0):
    """
    Blend three signal sources with platoon adjustment.
    Live Vegas odds (vig-removed) get 20% weight when available.
    Platoon split applied as final adjustment capped at ±4%.
    """
    w_vegas = (0.20 if vegas_is_live else 0.10) if vegas_prob is not None else 0.0
    ...
    # Apply platoon adjustment (capped at ±4%)
...
_MOBILE_KEYS = [
    ...
    "Vegas Implied %", "Vegas Is Live", "Model Edge",
