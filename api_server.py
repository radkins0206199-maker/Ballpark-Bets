...
def backfill_features():
    """Push feature data from predictions JSON into Supabase results table."""
    try:
        ...
        # Load predictions JSON — has full features for all generated games
        pred_path = os.path.join(os.path.dirname(__file__), "daily_predictions.json")
        if not os.path.exists(pred_path):
            return jsonify({"success": False, "error": "daily_predictions.json not found"})

        with open(pred_path) as f:
            preds = _json.load(f)

        all_games = preds.get('games', []) + preds.get('best_bets', [])
        # Deduplicate
        seen = set()
        games = []
        for g in all_games:
            key = (g.get('Home Team',''), g.get('Away Team',''))
            if key not in seen:
                seen.add(key)
                games.append(g)
        ...
        if not date or not games:
            return jsonify({"success": False, "error": "No games in predictions JSON"})
        ...
        return jsonify({
            "success": True,
            "date": date,
            "games_in_json": len(games),
            "updated": updated,
            "message": f"Updated {updated} Supabase rows with features from {date} predictions"
        })
    except Exception as e:
        import traceback
