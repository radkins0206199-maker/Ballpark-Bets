...
def run_predictions():
    ...
    try:
        ...
        import io, sys
        # Run synchronously so we can capture the output and any errors
        ...
        import json, os
        pred_file = os.path.join(os.path.dirname(__file__), "daily_predictions.json")
        games_count = 0
        pred_date = None
        ...
        return jsonify({
            ...
            "date": pred_date,
            "log_tail": output[-3000:] if output else "",
        })
    except Exception as e:
        import traceback
        return jsonify({"success": False, "error": str(e), "traceback": traceback.format_exc()[-2000:]})
...
@app.route("/api/mlb/capture-closing-lines", methods=["GET", "POST"])
def capture_closing_lines_endpoint():
    """
    Capture closing lines for games near first pitch, and compute CLV.

    Intended to be called on a schedule through the afternoon/evening
    (e.g. hourly from 12:00 to 22:00 ET). Each call only writes for games
    within ~90 minutes of starting and never overwrites an existing
    closing price, so repeated calls are safe and idempotent.
    """
...
@app.route("/api/mlb/clv-summary")
def clv_summary_endpoint():
    """Aggregate CLV on the model's picked side — the fastest read on real edge."""
    from mlb_daily_report import clv_summary
    try:
        days = int(request.args.get("days", 30))
        return jsonify({"success": True, "clv": clv_summary(days)})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
