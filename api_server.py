...
def trigger_retrain():
    """Manually trigger model retraining — for testing and verification."""
    try:
        from mlb_daily_report import retrain_model_if_needed, RETRAIN_FLAG
        # Clear the flag so retrain isn't skipped
        if os.path.exists(RETRAIN_FLAG):
            os.remove(RETRAIN_FLAG)
        retrain_model_if_needed()
        return jsonify({"success": True, "message": "Retrain triggered — check logs"})
    except Exception as e:
