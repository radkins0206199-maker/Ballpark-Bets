...
CRITICAL_DEFS = [
    ...
    "_acquire_run_lock", "_release_run_lock", "_safe_read_tracker_csv",
    "capture_closing_lines", "clv_summary", "_minutes_until_game",
    "_run_daily_predictions_inner",
...
def check_feature_pipeline():
    ...
    if "_acquire_run_lock()" in src and "finally:" in src:
        ok("run lock guards against concurrent pipeline runs")
    else:
        bad("no run lock — concurrent runs can corrupt shared files")
        failures.append("featpipe:no_lock")

    # Exactly one raw read is expected: the one inside _safe_read_tracker_csv,
    # which is guarded by existence/size checks and a try/except. Any others
    # are unguarded call sites that an empty CSV would crash.
    raw_reads = src.count("pd.read_csv(TRACKER_CSV")
    if raw_reads <= 1 and "_safe_read_tracker_csv" in src:
        ok(f"tracker CSV reads guarded ({raw_reads} raw read, inside safe reader)")
    else:
        bad(f"{raw_reads} raw pd.read_csv(TRACKER_CSV) calls — empty file will crash the run")
        failures.append("featpipe:unsafe_csv")

    # CLV capture must never overwrite a stored closing price, and must only
    # write near first pitch — a line captured at 9 AM is an opening line.
    if "closing_home_implied IS NULL" in src and "CLOSING_WINDOW_MIN" in src:
        ok("CLV capture is idempotent and time-windowed")
    else:
        bad("CLV capture missing idempotence or time window")
        failures.append("featpipe:clv")

    # CLV must be market-to-market (opening vs closing implied), never
    # model_prob vs closing. The latter just restates the model's own edge and
    # is positive by construction for an overconfident model — the first
    # implementation did this and reported a meaningless +18.7% with 100% of
    # picks "positive".
    if "home_win_prob" in src and "clv = round(model_home" in src:
        bad("CLV computed from model probability — that is model edge, not CLV")
        failures.append("featpipe:clv_definition")
    elif "opening_home_implied" in src and "cp - op" in src:
        ok("CLV is market-to-market (opening vs closing implied)")
    else:
        bad("could not verify CLV is market-to-market")
        failures.append("featpipe:clv_definition")
